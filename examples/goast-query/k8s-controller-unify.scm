;;; k8s-controller-unify.scm — Structural duplication across k8s controllers
;;;
;;; Loads deployment, replicaset, and statefulset controllers,
;;; then runs cross-package AST diff to find unification candidates.
;;;
;;; Usage: cd /path/to/kubernetes && wile-goast -f k8s-controller-unify.scm
;;;
;;; Requires: wile-goast binary, run from a directory where k8s packages resolve.
...
;; ══════════════════════════════════════════════════════════
;; Classification (must precede diff engine — called from ast-diff)
;; ══════════════════════════════════════════════════════════
...
;; ast-diff, fields-diff, list-diff are mutually recursive.
;; Defined together via internal defines to avoid forward references.
    ...
    (if (>= eff-sim similarity-threshold)
      (begin
        ...
        (for-each
          ...
          roots)
        ;; Show non-cosmetic, non-derived diffs
        (let ((interesting
                (filter (lambda (d)
                          (memq (car d) '(type-name literal-value
                                          operator structural
          ...
          (if (pair? interesting)
            (begin
              ...
              (for-each display-diff-entry
                        ;; Show at most 10 flagged diffs
                        (if (> (length interesting) 10)
                          (append
                            (let take ((xs interesting) (n 10))
                              (if (or (null? xs) (= n 0)) '()
...
(display "══════════════════════════════════════════════════")
(newline)
(display "  k8s Controller Structural Duplication Detection")
(newline)
(display "══════════════════════════════════════════════════")
(newline) (newline)

;; Load each target package individually
(define all-pkgs
  (flat-map
    (lambda (target)
...
;; Show signature shape distribution
(display "  Signature shapes:") (newline)
(let* ((shapes (map (lambda (e) (list-ref e 3)) all-funcs))
       (unique-shapes (unique shapes)))
  (for-each
    (lambda (shape)
      (let ((count (length (filter (lambda (s) (equal? s shape))
                                   shapes))))
        (display "    (") (display (car shape))
        (display " params, ") (display (cdr shape))
        (display " results): ") (display count)
